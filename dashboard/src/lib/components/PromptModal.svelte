...
<Modal id="new-prompt-modal" {open} {title} {onclose}>
    {#snippet titleBadge()}
    ...
    {:else}
        <form id="new-prompt-form" onsubmit={handleSubmit}>
            ...
            <div class="form-group">
                ...
                ></textarea>
                <small
                    >Optional JSON Schema for structured LLM output (passed directly in prompt to
                    avoid issues with models not supporting structured output).</small
                >
            </div>
            <div class="form-group">
