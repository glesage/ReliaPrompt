...
// Library mode API (code-first, read-only tests, ephemeral prompt drafts)
// Types come from backend: PromptSuiteDefinition, PromptDefinition, TestCaseDefinition from $backend/definitions/types
export type {
    PromptDefinition,
    TestCaseDefinition,
    PromptSuiteDefinition,
} from "$backend/definitions/types";

export async function getLibrarySuites(): Promise<PromptSuiteDefinition[]> {
    return fetchJSON<PromptSuiteDefinition[]>("/api/library/suites");
}

export async function runLibraryTest(data: {
    suiteId: string;
    promptDraft?: {
        content: string;
    ...
    testModels: SelectedModel[];
    evaluationModel?: SelectedModel;
    runsPerTest?: number;
}): Promise<{ score: number; results: LLMResult[] }> {
    return fetchJSON("/api/library/test/run", {
        method: "POST",
        body: JSON.stringify(data),
    });
}
